...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 16:41:50 2023
...
def main(file_number):
    ...
    filename = 'vessel_text_files/network_input.txt'
    ...
    # DIMAC pressure wave input 
    if (1):
        pressure_wave = np.loadtxt('ICA_pw_2_10_segments.txt')
        
    min_val = min(pressure_wave)
    max_val = max(pressure_wave)
    ...
    if (0):
        save_to_file_time_index = np.array([500, 1000, 1500, 2000])
        file_number = np.array([0, 1, 2, 3])
        ...
        path_save = 'output_values/ESV_outputs/dynamic_simulations/20_levels/50_segments/'
    
        # Path a
        # [number of time steps, file number]
        flow_ICA_ts_a = np.zeros([500, 4])
        flow_MCA_ts_a = np.zeros([500, 4])
        flow_level2_ts_a = np.zeros([500, 4])
        flow_level6_ts_a = np.zeros([500, 4])
        flow_level9_ts_a = np.zeros([500, 4])
    
        pressure_ICA_ts_a = np.zeros([500, 4])
        pressure_MCA_ts_a = np.zeros([500, 4])
        pressure_level2_ts_a = np.zeros([500, 4])
        pressure_level6_ts_a = np.zeros([500, 4])
        pressure_level9_ts_a = np.zeros([500, 4])
        
        # Path b 
        flow_ICA_ts_b = np.zeros([500, 4])
        flow_MCA_ts_b = np.zeros([500, 4])
        flow_level2_ts_b = np.zeros([500, 4])
        flow_level6_ts_b = np.zeros([500, 4])
        flow_level9_ts_b = np.zeros([500, 4])
    
        pressure_ICA_ts_b = np.zeros([500, 4])
        pressure_MCA_ts_b = np.zeros([500, 4])
        pressure_level2_ts_b = np.zeros([500, 4])
        pressure_level6_ts_b = np.zeros([500, 4])
        pressure_level9_ts_b = np.zeros([500, 4])
    ...
    #                resistance_ratio_matrix[t, :, segment] = remaining_resistance_ratios       
        ...
        ##################################################################################
        # Save pressure and flow values to file every 500s (for 20 level network)
        if (0):
            if t == save_to_file_time_index[n_file]: # save when t = 500, 1000, 1500 and 2000
                
                flow_ICA_ts_a = np.array([])
                flow_MCA_ts_a = np.array([])
                flow_level2_ts_a = np.array([])
                flow_level6_ts_a = np.array([])
                flow_level9_ts_a = np.array([])
                
                flow_ICA_ts_b = np.array([])
                flow_MCA_ts_b = np.array([])
                flow_level2_ts_b = np.array([])
                flow_level6_ts_b = np.array([])
                flow_level9_ts_b = np.array([])
     
                pressure_ICA_ts_a = np.array([])
                pressure_MCA_ts_a = np.array([])
                pressure_level2_ts_a = np.array([])
                pressure_level6_ts_a = np.array([])
                pressure_level9_ts_a = np.array([])
                
                pressure_ICA_ts_b = np.array([])
                pressure_MCA_ts_b = np.array([])
                pressure_level2_ts_b = np.array([])
                pressure_level6_ts_b = np.array([])
                pressure_level9_ts_b = np.array([])
    
     
                # finds values of pressure and flow between required time steps e.g. 0 and 500s, 500 and 1000s etc... 
                for time_t in range(m_file, save_to_file_time_index[n_file]):
                    
                    flow_ICA_ts_a = np.append(flow_ICA_ts_a, flow_matrix[time_t, 0, 0])
                    flow_MCA_ts_a = np.append(flow_MCA_ts_a, flow_matrix[time_t, 0, 1])
                    flow_level2_ts_a =  np.append(flow_level2_ts_a, flow_matrix[time_t, 0, 2])
                    flow_level6_ts_a = np.append(flow_level6_ts_a, flow_matrix[time_t, 0, 6])
                    flow_level9_ts_a = np.append(flow_level9_ts_a, flow_matrix[time_t, 0, 9])
        
                    pressure_ICA_ts_a = np.append(pressure_ICA_ts_a, pressure_matrix[time_t, 0, 0])
                    pressure_MCA_ts_a = np.append(pressure_MCA_ts_a, pressure_matrix[time_t, 0, 1])
                    pressure_level2_ts_a =  np.append(pressure_level2_ts_a, pressure_matrix[time_t, 0, 2])
                    pressure_level6_ts_a = np.append(pressure_level6_ts_a, pressure_matrix[time_t, 0, 6])
                    pressure_level9_ts_a = np.append(pressure_level9_ts_a, pressure_matrix[time_t, 0, 9])
        
                    flow_ICA_ts_b = np.append(flow_ICA_ts_b, flow_matrix[time_t, path_b_index[0], 0])
                    flow_MCA_ts_b = np.append(flow_MCA_ts_b, flow_matrix[time_t, path_b_index[1], 1])
                    flow_level2_ts_b =  np.append(flow_level2_ts_b, flow_matrix[time_t, path_b_index[2], 2])
                    flow_level6_ts_b = np.append(flow_level6_ts_b, flow_matrix[time_t, path_b_index[6], 6])
                    flow_level9_ts_b = np.append(flow_level9_ts_b, flow_matrix[time_t, path_b_index[9], 9])
        
                    pressure_ICA_ts_b = np.append(pressure_ICA_ts_b, pressure_matrix[time_t, path_b_index[0], 0])
                    pressure_MCA_ts_b = np.append(pressure_MCA_ts_b, pressure_matrix[time_t, path_b_index[1], 1])
                    pressure_level2_ts_b =  np.append(pressure_level2_ts_b, pressure_matrix[time_t, path_b_index[2], 2])
                    pressure_level6_ts_b = np.append(pressure_level6_ts_b, pressure_matrix[time_t, path_b_index[6], 6])
                    pressure_level9_ts_b = np.append(pressure_level9_ts_b, pressure_matrix[time_t, path_b_index[9], 9])
    
                  
                # save values for chosen time interval for paths a and b     
                pressure_ICA = np.zeros([500, 2])
                pressure_MCA = np.zeros([500, 2])
                pressure_level2 = np.zeros([500, 2])
                pressure_level6 = np.zeros([500, 2])
                pressure_level9 = np.zeros([500, 2])
                
                flow_ICA = np.zeros([500, 2])
                flow_MCA = np.zeros([500, 2])
                flow_level2 = np.zeros([500, 2])
                flow_level6 = np.zeros([500, 2])
                flow_level9 = np.zeros([500, 2])
    
                
                flow_ICA[:, 0] = flow_ICA_ts_a
                flow_MCA[:, 0] = flow_MCA_ts_a
                flow_level2[:, 0] = flow_level2_ts_a
                flow_level6[:, 0] = flow_level6_ts_a
                flow_level9[:, 0] = flow_level9_ts_a
                    
                flow_ICA[:, 1] = flow_ICA_ts_b
                flow_MCA[:, 1] = flow_MCA_ts_b
                flow_level2[:, 1] = flow_level2_ts_b
                flow_level6[:, 1] = flow_level6_ts_b
                flow_level9[:, 1] = flow_level9_ts_b
                
                pressure_ICA[:, 0] = pressure_ICA_ts_a
                pressure_MCA[:, 0] = pressure_MCA_ts_a
                pressure_level2[:, 0] = pressure_level2_ts_a
                pressure_level6[:, 0] = pressure_level6_ts_a
                pressure_level9[:, 0] = pressure_level9_ts_a
                    
                pressure_ICA[:, 1] = pressure_ICA_ts_b
                pressure_MCA[:, 1] = pressure_MCA_ts_b
                pressure_level2[:, 1] = pressure_level2_ts_b
                pressure_level6[:, 1] = pressure_level6_ts_b
                pressure_level9[:, 1] = pressure_level9_ts_b
    
                np.savetxt(path_save+'ICA_flow_MCA_compliance_'+str(file_number[n_file])+'.txt', flow_ICA)
                np.savetxt(path_save+'MCA_flow_MCA_compliance_'+str(file_number[n_file])+'.txt', flow_MCA)
                np.savetxt(path_save+'level2_flow_MCA_compliance_'+str(file_number[n_file])+'.txt', flow_level2)
                np.savetxt(path_save+'level6_flow_MCA_compliance_'+str(file_number[n_file])+'.txt', flow_level6)
                np.savetxt(path_save+'level9_flow_MCA_compliance_'+str(file_number[n_file])+'.txt', flow_level9)
            
                np.savetxt(path_save+'ICA_pressure_MCA_compliance_'+str(file_number[n_file])+'.txt', pressure_ICA)
                np.savetxt(path_save+'MCA_pressure_MCA_compliance_'+str(file_number[n_file])+'.txt', pressure_MCA)
                np.savetxt(path_save+'level2_pressure_MCA_compliance_'+str(file_number[n_file])+'.txt', pressure_level2)
                np.savetxt(path_save+'level6_pressure_MCA_compliance_'+str(file_number[n_file])+'.txt', pressure_level6)
                np.savetxt(path_save+'level9_pressure_MCA_compliance_'+str(file_number[n_file])+'.txt', pressure_level9)
    
                # updated index for next iteration
                m_file = save_to_file_time_index[n_file]             
                n_file = n_file+1
    ...
    if (1):
        ...
        for n in range(0, len(time)):
            ...
            #flow_level9_ts_a = np.append(flow_level9_ts_a, flow_matrix[n, 0, 9])
        ...
        for n in range(0, len(time)):
            ...
            #flow_level9_ts_b = np.append(flow_level9_ts_b, flow_matrix[n, path_b_index[9], 9])
        ...
        #flow_level9 = np.zeros([len(time), 2])
        ...
        # Pressure for every time point in ICA, MCA and capillaries (and corresponding levels for path a)
        pressure_ICA_ts_a = np.array([])
        pressure_MCA_ts_a = np.array([])
        pressure_cap_ts_a = np.array([])
        pressure_level2_ts_a = np.array([])
        #pressure_level9_ts_a = np.array([])
        for n in range(0, len(time)):
            pressure_ICA_ts_a = np.append(pressure_ICA_ts_a, pressure_matrix[n, 0, 0])
            pressure_MCA_ts_a = np.append(pressure_MCA_ts_a, pressure_matrix[n, 0, 1])
            pressure_cap_ts_a = np.append(pressure_cap_ts_a, pressure_matrix[n, 0, 6])
            pressure_level2_ts_a = np.append(pressure_level2_ts_a, pressure_matrix[n, 0, 2])
            #pressure_level9_ts_a = np.append(pressure_level9_ts_a, pressure_matrix[n, 0, 9])
    
        pressure_ICA_ts_b = np.array([])
        pressure_MCA_ts_b = np.array([])
        pressure_cap_ts_b = np.array([])
        pressure_level2_ts_b = np.array([])
        #pressure_level9_ts_b = np.array([])
        for n in range(0, len(time)):
            pressure_ICA_ts_b = np.append(pressure_ICA_ts_b, pressure_matrix[n, path_b_index[0], 0])
            pressure_MCA_ts_b = np.append(pressure_MCA_ts_b, pressure_matrix[n, path_b_index[1], 1])
            pressure_cap_ts_b = np.append(pressure_cap_ts_b, pressure_matrix[n, path_b_index[6], 6])
            pressure_level2_ts_b = np.append(pressure_level2_ts_b, pressure_matrix[n, path_b_index[2], 2])
            #pressure_level9_ts_b = np.append(pressure_level9_ts_b, pressure_matrix[n, path_b_index[9], 9])
            
        # save path b AND corresponding path a values 
        # (time points, path)
        # Column 0: path a, column 1: path b
        pressure_ICA = np.zeros([len(time), 2])
        pressure_MCA = np.zeros([len(time), 2])
        pressure_cap = np.zeros([len(time), 2])
        pressure_level2 = np.zeros([len(time), 2])
        #pressure_level9 = np.zeros([len(time), 2])
        
        pressure_ICA[:, 0] = pressure_ICA_ts_a
        pressure_MCA[:, 0] = pressure_MCA_ts_a
        pressure_cap[:, 0] = pressure_cap_ts_a
        pressure_level2[:, 0] = pressure_level2_ts_a
        #pressure_level9[:, 0] = pressure_level9_ts_a
            
        pressure_ICA[:, 1] = pressure_ICA_ts_b
        pressure_MCA[:, 1] = pressure_MCA_ts_b
        pressure_cap[:, 1] = pressure_cap_ts_b
        pressure_level2[:, 1] = pressure_level2_ts_b
        #pressure_level9[:, 1] = pressure_level9_ts_b
        

        path_outputs = 'output_values/ESV_outputs/DIMAC_temp/'
        
        # Save flow
        np.savetxt(path_outputs+'ICA_flow_DIMAC_'+str(file_number)+'.txt', flow_ICA)
        np.savetxt(path_outputs+'MCA_flow_DIMAC_'+str(file_number)+'.txt', flow_MCA)
        np.savetxt(path_outputs+'level2_flow_DIMAC_'+str(file_number)+'.txt', flow_level2)
        np.savetxt(path_outputs+'cap_flow_DIMAC_'+str(file_number)+'.txt', flow_cap)
    
        # Save pressure 
        np.savetxt(path_outputs+'ICA_pressure_DIMAC_'+str(file_number)+'.txt', pressure_ICA)
        np.savetxt(path_outputs+'MCA_pressure_DIMAC_'+str(file_number)+'.txt', pressure_MCA)
        np.savetxt(path_outputs+'level2_pressure_DIMAC_'+str(file_number)+'.txt', pressure_level2)
        np.savetxt(path_outputs+'cap_pressure_DIMAC_'+str(file_number)+'.txt', pressure_cap)

    
    if (0):
        path = 'output_values/ESV_outputs/dynamic_simulations/20_levels/50_segments/'
        
        np.savetxt(path+'ICA_flow_MCA_compliance.txt', flow_ICA)
        np.savetxt(path+'MCA_flow_MCA_compliance.txt', flow_MCA)
        np.savetxt(path+'level2_flow_MCA_compliance.txt', flow_level2)
        np.savetxt(path+'level6_flow_MCA_compliance.txt', flow_cap)
        np.savetxt(path+'level9_flow_MCA_compliance.txt', flow_level9)
    
    
        np.savetxt(path+'ICA_pressure_MCA_compliance.txt', pressure_ICA)
        np.savetxt(path+'MCA_pressure_MCA_compliance.txt', pressure_MCA)
        np.savetxt(path+'level2_pressure_MCA_compliance.txt', pressure_level2)
        np.savetxt(path+'level6_pressure_MCA_compliance.txt', pressure_cap)
        np.savetxt(path+'level9_pressure_MCA_compliance.txt', pressure_level9)
